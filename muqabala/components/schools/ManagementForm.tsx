...
type Field={name:string;label:string;value?:string;type?:'text'|'date'|'email';options?:{value:string;label:string}[]};
export function SchoolsManagementForm({operation,fields,fixed={},button}:{operation:string;fields:Field[];fixed?:Record<string,unknown>;button:string}){
  ...
  return <form method="post" className="schools-card" onSubmit={async event=>{event.preventDefault();setBusy(true);setMessage('');
    ...
    {fields.map(field=><label key={field.name}>{field.label}{field.options?<select name={field.name} required><option value="">Choose</option>{field.options.map(option=><option key={option.value} value={option.value}>{option.label}</option>)}</select>:<input name={field.name} defaultValue={field.value} type={field.type??'text'} required maxLength={500}/>}</label>)}
