...
export function SchoolsEnrol() {
  ...
  useEffect(()=>{if(!busy)return;const timer=setInterval(()=>setElapsed(value=>value+1),1000);return()=>clearInterval(timer);},[busy]);
  ...
  return <><h1>Join or recover your student account</h1><p>Use the private link your adviser gave you, or your saved recovery code. A cohort code cannot open an account.</p>
    <form method="post" className="schools-card" onSubmit={async event=>{event.preventDefault();setElapsed(0);setBusy(true);setError('');try{
