...
export function SchoolsPilotContact(){
  ...
  return <section id="start-pilot" className="schools-section"><h2>Start a pilot</h2><p>Tell us about your institution, adult students and teaching language. We reply personally; this form does not send an automatic confirmation email.</p>
    {!sent&&<form method="post" onSubmit={async event=>{event.preventDefault();const form=new FormData(event.currentTarget);setBusy(true);setMessage('');try{
      ...
      const body=await response.json();if(!response.ok)throw new Error(body.error);setSent(true);setMessage('Your pilot enquiry is saved. This form does not send an automatic confirmation email. We will contact you using the email you provided.');
      ...
      <label>Message<textarea name="message" rows={5} maxLength={2000} required/></label><button disabled={busy}>Send pilot enquiry</button>
