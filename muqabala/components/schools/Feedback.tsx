...
export function SchoolsFeedback({attemptId,status,detail,covered,assignmentId,rubrics,onRetry,retryBusy}:{attemptId:string;status:string;detail:unknown;covered:number|null;assignmentId?:string;rubrics?:{id:string;label:string}[][];onRetry?:(question:number)=>void;retryBusy?:boolean}) {
  ...
  const [busy,setBusy]=useState(status!=='ready'),[message,setMessage]=useState(status==='ready'?'':'Preparing your feedback. Allow up to two minutes. Your answers are saved.');
  ...
  if(!ready)return <section className="schools-card" aria-busy={busy}><h2>Your feedback</h2>
    ...
    {busy?<progress aria-label="Preparing your feedback"/>:<button onClick={()=>{setBusy(true);setMessage('Preparing your feedback. Allow up to two minutes. Your answers are saved.');setCycle(value=>value+1);}}>Check feedback again</button>}
    </section>;
  ...
  const raw=Array.isArray(resolvedDetail)?resolvedDetail.map(q=>({...q,elements:q.elements?.map((e:Record<string,unknown>)=>({id:e.id,present:e.present,supportingText:e.supportingText,confidence:e.confidence}))})):null;
