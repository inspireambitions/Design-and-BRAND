...
export function SchoolsFeedback({attemptId,status,detail,covered,assignmentId}:{attemptId:string;status:string;detail:unknown;covered:number|null;assignmentId?:string}) {
  ...
  return <section className="schools-card"><h2>Your private feedback</h2><p>Evidence covered: {covered} of 12 elements</p>
    ...
    {parsed.data.questions.map(q=><section key={q.questionIndex}><h3>Question {q.questionIndex+1}</h3><ul>{q.elements.map(e=><li key={e.id}>
      <strong>{e.id}: {e.present?'Present':'Absent'}</strong>. Engine confidence: {e.confidence}.
