...
export function RoleHelpPanel({ roleId, roleTitle, unreviewed, unresolvedQuestions, expiresAt, timezone }: { roleId: string; roleTitle: string; unreviewed: number; unresolvedQuestions: number; expiresAt: string; timezone: string }) {
  ...
  const questions = [
    { label: 'Which submissions still need review?', answer: { text: `${unreviewed} ${unreviewed === 1 ? 'submission still needs' : 'submissions still need'} human review.`, href: `/employer/roles/${roleId}?candidateStatus=unreviewed#submissions`, action: 'Open review queue' } },
    ...
    { label: 'What needs my attention?', answer: { text: `${unreviewed} awaiting review and ${unresolvedQuestions} unresolved candidate ${unresolvedQuestions === 1 ? 'question' : 'questions'}.`, href: unresolvedQuestions ? `/employer/roles/${roleId}#candidate-questions` : `/employer/roles/${roleId}#submissions`, action: 'Open next task' } },
  ];
