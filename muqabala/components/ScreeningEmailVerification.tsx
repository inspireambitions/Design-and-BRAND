...
type Props = { publicCode: string; companyName: string; roleTitle: string; roleTitleAr: string };

export function ScreeningEmailVerification({ publicCode, companyName, roleTitle, roleTitleAr }: Props) {
  ...
  const copy = lang === 'ar' ? {
    ...
    email: 'بريدك الإلكتروني في مقابلة', send: 'إرسال الرمز', sending: 'جارٍ الإرسال…',
    ...
    email: 'Your Muqabala email', send: 'Send 6-digit code', sending: 'Sending…',
      ...
      <div className={styles.shell}>
        ...
        <section className={styles.card}>
          ...
          {supported && <>
            ...
            <form className={styles.verifyForm} onSubmit={(event) => { event.preventDefault(); void requestCode(); }}>
            ...
            {message && <div className={styles.savedBanner} role="status">{message}</div>}
