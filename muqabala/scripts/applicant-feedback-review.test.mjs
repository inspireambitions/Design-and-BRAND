...
test('role cards describe and link to the one-question entry', () => {
  for (const lang of ['en', 'ar']) {
    const view = load('HomeView', 'HomeView', lang)({ roles: [{
      id: 'nurse', title: 'Staff Nurse', titleAr: 'ممرض', industry: 'Care', industryAr: 'رعاية',
      blurb: 'Ward nursing', blurbAr: 'تمريض', questionCount: 5, quickQuestionId: 'patient-care',
    }] });
    const caption = lang === 'ar' ? 'تدريب سريع: سؤال واحد' : 'Quick practice: one question';
    assert.equal(nodes(view, (n) => n.props?.children === caption).length, 1);
    assert.equal(nodes(view, (n) => n.props?.href === '/practice/nurse?focus=patient-care').length, 1);
  }
});
