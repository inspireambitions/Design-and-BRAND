...
qaStage('staging student login');
const student=await login('student'),{fixture:f,admin}=student;
qaStage('protected preview access');
...
try{
  ...
  for(let n=0;n<5;n++){
    qaStage('create synthetic assignment '+(n+1));
    const created=await admin.rpc('schools_manage_action',{actor:f.users.educator.id,operation:'assignment',device:'server',payload:{cohortId:f.cohorts[0],roleId:'Student placement',questionIds:links.data.map(q=>q.question_version_id),dueAt:new Date(Date.now()+(n+2)*86400000).toISOString()}});
    assert(!created.error);assignmentIds.push(created.data.id);attemptIds.push(randomUUID());
  }
  qaStage('five simultaneous feedback journeys');
  ...
  qaStage('five usage receipts');
  const usage=await admin.from('schools_feedback_versions').select('assignment_attempt_id,input_tokens,output_tokens').in('assignment_attempt_id',attemptIds);assert(!usage.error);
  assert.equal(usage.data.length,5,'Every model call needs a usage receipt');
  const inputTokens=usage.data.reduce((sum,item)=>sum+(item.input_tokens??0),0);
  const outputTokens=usage.data.reduce((sum,item)=>sum+(item.output_tokens??0),0);
  const evidence={checkedAt:new Date().toISOString(),startedAt,stagingRef:f.stagingRef,previewOrigin,syntheticOnly:true,model:'gpt-4.1-mini',pricingUsdPerMillion:{input:0.40,output:1.60,source:'https://developers.openai.com/api/docs/models/gpt-4.1-mini'},modelCallCap:5,modelCalls:usage.data.length,results:journeys.map(({claimId,...item})=>item),usage:{inputTokens,outputTokens,estimatedUsd:Number(((inputTokens*0.40+outputTokens*1.60)/1000000).toFixed(8)),calculatedFromStoredTokens:true},limitation:'Five simultaneous synthetic journeys validate this preview path only. They do not establish classroom-scale or provider-wide capacity.'};
