...
test('feedback endpoint is same-origin, bounded and sends only to the Muqabala inbox', () => {
  const route = read('app/api/feedback/route.ts');
  const schema = read('lib/rating-feedback.ts');
  assert.match(route, /hasTrustedOrigin/);
  assert.match(route, /MAX_BODY_BYTES = 2_000/);
  assert.match(route, /hello@trymuqabala\.com/);
  assert.match(route, /RESEND_FEEDBACK_API_KEY/);
  assert.match(route, /limitFeedback/);
  assert.doesNotMatch(schema, /transcript|audio|video|candidateName|emailAddress/);
});
