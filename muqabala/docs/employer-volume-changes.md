...
There is no `roles` table. A role is a row in `public.screening_packs`. Each new pack holds one signed eight-question role plan (`signed_token`, kind `proof`), the company name (`workplace`), an optional recruiter name inside the token, a public code, an expiry, a candidate capacity (`max_candidates`, `starts_used`) and the owning `employer_id`. Older signed three-question packs remain valid. Everything in this brief that says `role_id` maps to `screening_packs.id`.

Packs are created from `/for-employers` in three calls: `POST /api/screening/job-description` (optional draft), `POST /api/interview` (tailored eight-question interview from the advert, returns a practice token), then `POST /api/screening/packs`, which signs the eight-question role plan as a proof pack and inserts the row. The candidate link is `${origin}/s/${public_code}`.

### Candidate flow

`/s/[code]` resolves the pack, asks the candidate to verify an email (`/api/screening/auth/*`), then runs `EmployerVideoInterview`: name, device check, adaptive English video answers of up to 120 seconds, consent (`employer-video-v1`), submit. The Universal Interview Brain extracts evidence and chooses the next main question or follow-up after each saved recording. Arabic uses the signed eight-question plan without adaptive follow-ups. Older interviews that already started keep their original three-question flow. Writes go through service-role RPCs: `start_screening_interview` (11-argument overload, keyed by `(pack, candidate_user_id)` and an idempotency hash), `save_screening_video_answer`, `submit_screening_interview`. Videos live in the private `screening-videos` bucket. Submit sets `expires_at` to 90 days and enqueues two rows in `screening_notification_outbox` (candidate receipt, employer alert).
...
### Section 4: shortlist email, review screen, share

Committed. **Coverage** (`lib/employer-volume/coverage.ts`): the role's first four competencies, a tick when any answer holds non-null evidence for that competency, otherwise a cross. Never a number. Candidates rank full coverage first, then coverage count, then earliest submission (`lib/server/employer-candidates.ts`).

**Shortlist email**: queued 48 hours after the first invite batch and again when the role closes, only when at least one submission exists, guarded by two new `screening_packs` timestamps. Subject "[Role]: [answered] answered, [full coverage] to review". Up to ten rows with ticks, the first 90 characters of the first answer, and an Open link. Each Open link is a Supabase magic link (`auth.admin.generateLink`) whose `next` is that candidate's review page, so it signs the employer in and lands there with no password. Preview at `/dev/email/shortlist` (`?text=1` for the plain part), non-production only; there was no email client preview tooling to reuse.

**Review screen**: `/employer/interviews/[id]` keeps its route and, with the flag on, renders `CandidateReview`: name or reference, rubric ticks with labels, three collapsible transcripts (first open) with play-on-tap video, a fixed bottom bar with Shortlist, Pass, Later and an optional note, Next button and swipe left, centred at 640px on desktop. Decisions write to the new `employer_decisions` log (reviewer, time, decision, note) and mirror onto the legacy `interviews.employer_decision` column so dashboard counts still work. A 10 second toast offers Undo, which deletes the log row and restores the previous decision. Opening a candidate stamps `employer_reviewed_at`.
