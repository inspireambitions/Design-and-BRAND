...
- eight-question planning with code-owned slot types, frameworks and targets
- encrypted Evidence Ledger, coverage matrix, transcript store and interview state
- separate account-link table, 12-month provisional retention and immediate candidate deletion
...
1. Confirm the five open product decisions below.
2. Apply `20260902120000_universal_interview_brain_v2.sql` to an isolated staging Supabase project.
3. Add `UNIVERSAL_INTERVIEW_DATA_KEY` as a server-only secret.
...
1. Retention is provisionally 12 months.
2. ENTRY interviews currently use eight questions.
3. P1, P2, T1, T2 and F1 default to `OPENAI_SCORING_MODEL`. Each can be set separately.
4. The Front Desk Agent pack has no named subject expert or review date, so technical correctness is not scored.
5. The single retry is currently free.
...
- Fatima: candidate identity is separated from encrypted practice state. Immediate deletion exists. The retention period and legal notices still need final approval.
