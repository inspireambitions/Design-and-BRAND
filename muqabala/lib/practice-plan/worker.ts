...
import { buildPlanLinks } from './plan';
...
export type PlanSnapshot = { plan: SevenDayPlan; viewToken: string };
...
export function providerIdempotencyKey(requestId: string, day: number): string {
  return `practice-plan/${requestId}/day-${day}`;
}

/**
 * Sends the day's email for each claimed outbox job. One request has seven
 * jobs, one per day, released by `available_at`. A job is idempotent on
 * (request, day): if an attempt for that day was already accepted by the
 * provider the job completes without sending again.
 */
export async function processPracticePlanJobs(provider: EmailProvider = productionEmailProvider()) {
  ...
  for (const job of jobs ?? []) {
    const day = Number(job.plan_day ?? 1);
    const { data: request, error } = await admin.from('practice_plan_requests')
      .select('id,email_hash,email_ciphertext,locale,status')
    ...
    const { data: accepted } = await admin.from('email_delivery_attempts')
      .select('id')
    ...
    if (suppressed) {
      await Promise.all([
        ...
        admin.from('transactional_outbox').update({ state: 'dead_letter', locked_at: null, last_error_code: 'suppressed' }).eq('plan_request_id', request.id).in('state', ['pending', 'retry', 'processing']),
      ]);
    ...
    try {
      ...
      const snapshot = decryptJson<PlanSnapshot>(storedSnapshot.plan_ciphertext);
      const plan = SevenDayPlanSchema.parse(snapshot.plan);
      const links = buildPlanLinks(configuredOrigin(), snapshot.viewToken, plan);
      ...
      const sent = await provider.send({
        to: email,
        from: process.env.PRACTICE_PLAN_FROM || 'Muqabala <plan@trymuqabala.com>',
        ...rendered,
        idempotencyKey: providerIdempotencyKey(request.id, day),
      });
      await admin.from('email_delivery_attempts').update({
        status: 'accepted', provider_message_id: sent.providerMessageId,
      }).eq('plan_request_id', request.id).eq('plan_day', day).eq('attempt_number', job.attempt_number);
      await admin.from('practice_plan_requests').update({
        status: 'sent', last_event_at: new Date().toISOString(), last_sent_day: day,
      }).eq('id', request.id).in('status', ['queued', 'sending', 'sent', 'delivered', 'delayed', 'failed']);
      ...
      await Promise.all([
        exhausted ? admin.from('practice_plan_requests').update({ status: 'dead_letter' }).eq('id', request.id) : Promise.resolve(),
