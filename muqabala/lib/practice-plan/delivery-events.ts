...
/**
 * A plan is seven emails, so a provider message id is resolved through the
 * delivery attempt that sent it. The request-level column is kept as a
 * fallback for rows written by the single-email version of the pipeline.
 */
async function requestForMessage(admin: SupabaseClient, providerMessageId: string) {
  const { data: attempt } = await admin.from('email_delivery_attempts')
    .select('plan_request_id')
    .eq('provider_message_id', providerMessageId)
    .maybeSingle();
  const query = admin.from('practice_plan_requests').select('id,email_hash,status,last_event_at');
  const { data } = attempt
    ? await query.eq('id', attempt.plan_request_id).maybeSingle()
    : await query.eq('provider_message_id', providerMessageId).maybeSingle();
  return data;
}

export async function applyStoredResendEvents(admin: SupabaseClient, providerMessageId: string) {
  const request = await requestForMessage(admin, providerMessageId);
  if (!request) return;
