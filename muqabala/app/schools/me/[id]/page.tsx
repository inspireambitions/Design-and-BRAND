...
export default async function AssignmentPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{retry?:string}>}) {
  ...
  const {data:assignment}=await client.from('schools_assignments').select('id,cohort_id,due_at,role_id,job_title').eq('id',id).maybeSingle();
  ...
  return <><h1>{assignment.job_title||assignment.role_id}</h1><p>{questions.length} questions. Use examples from your own experience.</p>
