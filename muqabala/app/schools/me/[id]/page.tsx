...
export default async function AssignmentPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{retry?:string}>}) {
  ...
  const {data:versions}=await client.from('schools_question_versions').select('id,question_text,no_example_follow_up').in('id',links.map(l=>l.question_version_id));
  ...
  return <><h1>{assignment.role_id}</h1><p>Three questions. Use examples from your own experience.</p>
    <SchoolsPractice assignmentId={id} cohortId={assignment.cohort_id} questions={questions.map(q=>({text:q!.question_text,followUp:q!.no_example_follow_up}))}
