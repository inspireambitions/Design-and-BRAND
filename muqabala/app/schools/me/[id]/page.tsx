...
import { SchoolsAdaptivePractice } from '@/components/schools/AdaptivePractice';
...
export default async function AssignmentPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{retry?:string}>}) {
  ...
  if(error||!links||links.length<3||links.length>8) throw new Error('Assignment questions are not available');
  ...
  const {data:attempts}=await client.from('schools_assignment_attempts').select('id,status,answers,revision,attempt_number,feedback_status,evidence_detail,evidence_covered,universal_interview_id,delivery_mode').eq('assignment_id',id).eq('student_user_id',user.id).order('attempt_number',{ascending:false}).limit(1);
