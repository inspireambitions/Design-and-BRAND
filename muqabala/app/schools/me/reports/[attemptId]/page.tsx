...
export default async function StudentReport({params}:{params:Promise<{attemptId:string}>}) {
  ...
  const {data:links}=await client.from('schools_assignment_questions').select('question_index,question_version_id').eq('assignment_id',attempt.assignment_id).order('question_index');
  const {data:versions}=links?.length?await client.from('schools_question_versions').select('id,rubric').in('id',links.map(link=>link.question_version_id)):{data:[]};
  const rubrics=links?.map(link=>versions?.find(version=>version.id===link.question_version_id)?.rubric??[]);
  ...
  const prevAttempt=attempt.attempt_number>1?attempts?.slice().reverse().find(item=>item.attempt_number<attempt.attempt_number):null;
