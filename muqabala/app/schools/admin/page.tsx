...
export default async function InstitutionAdmin(){
  ...
  const educatorLabels=new Map<string,string>();
  for(const educator of educators??[]){const identity=await admin.auth.admin.getUserById(educator.user_id);educatorLabels.set(educator.user_id,identity.data.user?.email??'Email unavailable');}
  ...
  return <><h1>Your institution</h1>{institutions?.map(i=><section className="schools-card" key={i.id}><h2>{i.name}</h2><p>{i.country}. Teaching language: {i.language==='en'?'English':'Arabic'}.</p>
    ...
    <SchoolsManagementForm operation="institution_details" fixed={{institutionId:i.id}} button="Update institution details" fields={[{name:'name',label:'Institution name',value:i.name},{name:'country',label:'Country',value:i.country}]}/>
    ...
    {cohorts?.filter(c=>c.institution_id===i.id).map(c=><section key={c.id}><h3>{c.name}</h3><p>{members?.filter(m=>m.cohort_id===c.id&&m.status==='active').length??0} active students.</p>
      <SchoolsManagementForm operation="assign_educator" fixed={{cohortId:c.id}} button="Assign educator" fields={[{name:'userId',label:'Accepted educator account',options:(educators??[]).filter(e=>e.institution_id===i.id).map(e=>({value:e.user_id,label:educatorLabels.get(e.user_id)!}))}]}/></section>)}
    </section>)}</>;
}
