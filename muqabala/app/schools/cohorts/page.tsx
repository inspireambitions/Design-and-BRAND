...
export default async function CohortsPage() {
  ...
  const [cohortsResult, membersResult, assignmentsResult] = await Promise.all([
    client.from('schools_cohorts').select('id,name,programme,faculty,campus,programme_id').in('id', cohortIds),
              ...
              <div className="schools-dashboard-card-heading">
                <div>
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '4px' }}>
                    <p className="schools-eyebrow" style={{ margin: 0 }}>Cohort</p>
                    {detail?.programme && (
                    ...
                    {detail?.campus && (
                      <span className="schools-state" style={{ background: '#f3f4f6', color: '#374151', fontSize: '11px', padding: '2px 6px' }}>
                        {detail.campus}
                      </span>
                    )}
                  </div>
