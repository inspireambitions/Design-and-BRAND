...
export async function GET(request: Request) {
  ...
  try {
    const result = await drainScreeningNotifications(() => processScreeningNotifications({ limit: 5 }));
    ...
    if (result.failed) {
      reportOperationalFailure('cron_job_failed', { area: 'cron', job: 'screening_notifications', code: 'notification_retry_required', status: 503 });
    }
    reportOperationalEvent('cron_job_completed', { area: 'cron', job: 'screening_notifications', code: result.stopped, status: result.failed ? 503 : 200, count: result.accepted });
