...
export async function POST(request: Request) {
  ...
  if (!secret || !apiKey) return Response.json({ error: 'Unavailable.' }, { status: 503 });
  ...
  let event;
  try {
    event = new Resend(apiKey).webhooks.verify({ payload, headers: { id: eventId, timestamp, signature }, webhookSecret: secret });
  } catch {
    return Response.json({ error: 'Invalid webhook.' }, { status: 400 });
  }
  if (!supportedResendEvents.has(event.type) || !('email_id' in event.data)) return Response.json({ received: true });
