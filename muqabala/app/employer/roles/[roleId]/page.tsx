...
export default async function EmployerRolePage({ params, searchParams }: { params: Promise<{ roleId: string }>; searchParams: Promise<{ candidateStatus?: string | string[] }> }) {
  ...
  const [visibleResult, totalResult, unreviewedResult, shortlistResult, inviteResult, questionResult, unresolvedResult] = await Promise.all([
    ...
    client.from('candidate_role_questions').select('id,role_id,candidate_email,question_text,answer_already_shown,reply_text,replied_at,resolved_at,created_at').eq('role_id', roleId).order('created_at', { ascending: false }).limit(100),
  ...
  return <EmployerReviewPanelProvider><main className={styles.page}>
    ...
    <section className={styles.section} id="candidate-questions" aria-labelledby="questions-title"><header><div><h2 id="questions-title">Candidate questions</h2>{unresolvedCount === null ? <p>Question activity is temporarily unavailable.</p> : <p>{unresolvedCount ? `${unresolvedCount} unresolved` : 'No unresolved questions'}. Replying and resolving are separate.</p>}</div>{!questionUnavailable && questions.length > 0 && <Link href="/employer/questions">Combined queue</Link>}</header>{questionUnavailable ? <RetryState>Candidate questions could not be loaded.</RetryState> : <RecruiterQuestions questions={questions} />}</section>

    <section className={styles.section} id="submissions" aria-labelledby="submissions-title"><header><div><h2 id="submissions-title">Submissions</h2>{submissionUnavailable ? <p>Submission activity is temporarily unavailable.</p> : <p>{submissionCount} submitted · {unreviewedCount} awaiting review · {shortlistCount} shortlisted</p>}</div>{!submissionUnavailable && <nav aria-label="Filter submissions"><Link href={`/employer/roles/${roleId}#submissions`} aria-current={!requestedStatus ? 'page' : undefined}>All</Link><Link href={`/employer/roles/${roleId}?candidateStatus=unreviewed#submissions`} aria-current={requestedStatus === 'unreviewed' ? 'page' : undefined}>Awaiting review</Link><Link href={`/employer/roles/${roleId}?candidateStatus=shortlisted#submissions`} aria-current={requestedStatus === 'shortlisted' ? 'page' : undefined}>Shortlist</Link></nav>}</header>{submissionUnavailable ? <RetryState>Submissions could not be loaded.</RetryState> : <div className={styles.submissions}>{submissions.map((submission) => <article key={submission.id}><div><strong dir="auto">{submission.candidate_name || 'Candidate'}</strong><p>Submitted {new Date(submission.submitted_at).toLocaleString('en-GB')} · {submission.employer_reviewed_at ? 'Reviewed' : 'Awaiting review'}</p></div><EmployerReviewTrigger interviewId={submission.id} candidateLabel={submission.candidate_name || 'Candidate'} className={styles.review}>Review submission</EmployerReviewTrigger><DashboardDecisionActions interviewId={submission.id} candidateLabel={submission.candidate_name || 'Candidate'} currentDecision={submission.employer_decision} /></article>)}{submissions.length === 0 && <p className={styles.empty}>No submissions match this filter.</p>}{submissions.length === 50 && <p className={styles.empty}>Showing the 50 most recent matching submissions.</p>}</div>}</section>
